...
const MarketTicker: React.FC = () => {
  ...
  const getColor = (change: number) => (change > 0 ? 'text-green' : 'text-red');
  const getIconColor = (change: number) => (change > 0 ? '#5cdb95' : '#d35a35');
...
<LinearGradient
      ...
      <Animated.View className="flex-row items-center space-x-4" style={animatedStyle}>
        <Text className="font-medium px-2 text-black">
